...
EXP_DATADIR = "experiment_data/hevc_tile_mapping_bg_varHops_normal/"
...
def runSim_TileMapping_varBGMappingNHops(    
    ...
    if(_check_file_exists(check_fname) == True):        
        ...
        print ">> Simulation Starting : WallClock DateTime: " + time.strftime("%c")
        ...
        print ">> Everything Finished : WallClock DateTime: " + time.strftime("%c")
...
parser.add_argument("--memp_select", help="memory port selector", type=int, default=2)
parser.add_argument("--nhgt_h", help="num hops NGT - high ccr", type=int, default=-1)
parser.add_argument("--nhgt_m", help="num hops NGT - med ccr", type=int, default=-1)
parser.add_argument("--nhgt_l", help="num hops NGT - low ccr", type=int, default=-1)
...
if (args.wl_config == None) or (args.forced_seed == -1) or \
    (args.cmbmppri_type == -1) or (args.memp_select==-1) or \
    (args.nhgt_h==-1) or (args.nhgt_m==-1) or (args.nhgt_l==-1):
        sys.exit("Arguments invalid")


# construct filename
runSim_TileMapping_varBGMappingNHops(
