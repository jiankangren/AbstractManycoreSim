...
#        sys.exit("Error - no wf id specified")
    ...
    if(wf_id != None):
        ...
        leg = ax.legend( (rects0[0], rects1[0], rects2[0], ), 
        ...
        if(plot3D == True):
            ...
            for each_wf_id in WF_RANGE:
                ...
                streams_late = [x["num_vids_accepted_late"] for x in each_wf_data.values()]
                streams_rejected = [x["num_vids_rejected"] for x in each_wf_data.values()]
                gops_dropped = [float(x["num_dropped_tasks"]/12.0) for x in each_wf_data.values()]
...
#                    l = plt.axvline(x=(width*3.5)*x)
                ...
                rects2 = axarr[i].bar(positions[2], streams_rejected, width=width, color='r', alpha=0.9)
                rects3 = axarr[i].bar(positions[3], gops_dropped, width=width, color='y', alpha=0.9)  
                
                _autolabel(rects0, axarr[i])
                _autolabel(rects1, axarr[i])
...
def plot_GOPLateness_VH():
    ...
    for wf_id in NUM_WORKFLOWS:
        ...
        for each_exp_key, each_exp_data in all_exp_data[wf_id].iteritems():
            ...
            for each_ugid_key, each_ugid_val in each_exp_data.iteritems():
                ...
                track_gop_lateness.append(each_ugid_val['gop_execution_lateness'])
    ...
    print "gop_lateness_sum = " + str(np.sum(track_gop_lateness))
    print "gop_lateness_mean = " + str(np.mean(track_gop_lateness))
    ...
    for wf_id in WF_RANGE:
        ...
        print "---"
        print "Number of gops late :" + str(len(allwfs_goplateness_allperms[wf_id][0]))
        pprint.pprint(allwfs_goplateness_allperms[wf_id][0])
        print "---"
