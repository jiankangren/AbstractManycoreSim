...
#             exp_data['DEQF-AC'] = _getEntry(file_data)
            ...
            for each_ratio_combo in RATIO_COMBINATIONS:
                ...
                key =  "(" + str(each_ibuff_ratio) + "," + str(each_tq_ratio) + ")"
                
                exp_data[key] = _getEntry(file_data)        
...
#        _set_bp(axarr[i],bp2, 'r' )
        ...
        rects0 = axarr[i].bar(positions[0], streams_schedulable, width=width, color='g', alpha=0.9, label="Admitted and fully schedulable")                
        rects1 = axarr[i].bar(positions[1], streams_late, width=width, color='b', alpha=0.9, label="Admitted but late")                
        rects2 = axarr[i].bar(positions[2], streams_rejected, width=width, color='#FF4747', alpha=0.9, label="Rejected")  
        ...
        axarr[i-1].tick_params(axis='both', which='major', labelsize=10)
        
        
        i+= 1
   
    #axarr[i-1].set_xlabel("Admission control test type", fontsize=14)
    #f.text(0.02, 0.5, 'Number of video streams', va='center', rotation='vertical', fontsize=14)
    ...
    axarr[i-1].set_xticklabels( labels , rotation=35, fontsize=10)
    #t[0].set_y(1.09)  
    plt.axis('tight')
...
# - heu based
# - sched based
def plot_ACTest_AllMetrics_HeuVar(wf_id = None, plot3D = True):

    
    ##############################
    ####### gather results 
    ##############################    
    all_wf_exp_adminrate_data = {}
    all_wf_exp_util_data = {}
    
    #F_PREFIX = "_vsbs.js"
    
    
    for each_wf_id in NUM_WORKFLOWS:
        all_wf_exp_adminrate_data[each_wf_id] = {}
        all_wf_exp_util_data[each_wf_id] = {}
        
        for each_seed in RANDOM_SEED_LIST:        
        
            exp_adminrate_data = OrderedDict()      # dict that will contain all the results for each param  
            exp_util_data = OrderedDict()      # dict that will contain all the results for each param
            ...
            for each_ratio_combo in RATIO_COMBINATIONS:
                ...
                # get adminrate data               
                F_SUFFIX = ".js" 
                vs_bs_fname = EXP_DATA_FOLDER + "seed_" + str(each_seed) + "/" + FNAME_PREFIX + 'wf'+str(each_wf_id)+'_cores'+str(EXP_NUM_NODES) + F_SUFFIX
                print vs_bs_fname
                json_data=open(vs_bs_fname)
                file_data = json.load(json_data)                
                key =  "(" + str(each_ibuff_ratio) + "," + str(each_tq_ratio) + ")"                
                exp_adminrate_data[key] = _getEntry(file_data)      
                
                # get util data
                F_SUFFIX = "__utilisation.js"           
                vs_bs_fname = EXP_DATA_FOLDER + "seed_" + str(each_seed) + "/" + FNAME_PREFIX + 'wf'+str(each_wf_id)+'_cores'+str(EXP_NUM_NODES) + F_SUFFIX
                print vs_bs_fname
                json_data=open(vs_bs_fname)
                file_data = json.load(json_data)                
                #key =  "(" + str(each_ibuff_ratio) + "," + str(each_tq_ratio) + ")"                
                #total_sim_time = float(len(file_data['node_idle_time']) * 0.5 * 1.0)
                #idle_time_percentage = np.mean(file_data['node_idle_time'][-1]['it_c'])/total_sim_time
                
                exp_util_data[key] = file_data
            
            all_wf_exp_adminrate_data[each_wf_id]['seed_'+str(each_seed)] = exp_adminrate_data 
            all_wf_exp_util_data[each_wf_id]['seed_'+str(each_seed)] = exp_util_data
    ...
    for wf_id in NUM_WORKFLOWS:
        ...
        for each_exp_seed_key, each_exp_seed_data in all_wf_exp_util_data[wf_id].iteritems():                            
    ...
    labels = [('%s'%x) for x in exp_adminrate_data.keys()] 
    ##############################
    ####### plot results
    ##############################    
    ...
    f.canvas.set_window_title("All metric results")
        
        
    each_wf_adminrate_data = all_wf_exp_adminrate_data[each_wf_id]
    each_wf_util_data = all_wf_exp_util_data[each_wf_id]
    
    # --------- number of success & failed videos ----------          
        
    # data for different results categories
    streams_schedulable = _get_multi_seed_data_vidstats('num_vids_accepted_success', each_wf_adminrate_data, mean=True)
    streams_late = _get_multi_seed_data_vidstats('num_vids_accepted_late', each_wf_adminrate_data, mean=True)
    streams_rejected = _get_multi_seed_data_vidstats('num_vids_rejected', each_wf_adminrate_data, mean=True)        
    #dropped_gops = _get_multi_seed_data_vidstats('num_dropped_tasks', each_wf_data, mean=True)        
    ...
    rects0 = axarr[0].bar(positions[0], streams_schedulable, width=width, color='g', alpha=0.9, label="Admitted and fully schedulable")                
    rects1 = axarr[0].bar(positions[1], streams_late, width=width, color='b', alpha=0.9, label="Admitted but late")                
    rects2 = axarr[0].bar(positions[2], streams_rejected, width=width, color='#FF4747', alpha=0.9, label="Rejected")  
     
    axarr[0].grid(True, alpha=0.5)        
    axarr[0].tick_params(axis='both', which='major', labelsize=10)
    #axarr[0].set_xlabel("Admission control test type", fontsize=12)
    axarr[0].set_ylabel("Number of\nvideo streams", fontsize=12)
    axarr[0].set_xticks(ind+(width*1.5))
    
    leg1 = axarr[0].legend(ncol=3, loc = 'upper center', fontsize=12, 
                         bbox_to_anchor = (0,0.02,1,1), bbox_transform = plt.gcf().transFigure
                )
    ...
    axarr[1].bar(positions, summation[wf_id], width=width, color=temp_cols, alpha=0.9)    
    
    
    axarr[1].set_xticks(ind+(width*1.5))
    axarr[1].tick_params(axis='both', which='major', labelsize=10)
    axarr[1].set_xticklabels( labels , rotation=35, fontsize=10)
    axarr[1].set_xlabel(r"Heu-AC admission control test type $(IBL_{\alpha}, TQL_{\alpha})$ ", fontsize=12)
    axarr[1].set_ylabel("Mean\nPE busy time %", fontsize=12)
    #t[0].set_y(1.09)  
    #plt.axis('tight')
...
def _autolabel(rects, ax, extra_text=None, fsize=12):
    j=0
    for rect in rects:
...
#             exp_data['Deterministic.\n(comp.)'] = file_data
    ...
    for wf_id in WF_RANGE:
        ...
        axarr[i].tick_params(axis='x', which='both', labelsize=12, labelcolor='k')
        axarr[i].tick_params(axis='y', which='both', labelsize=12, labelcolor='b')
...
plot_ACTest_AllMetrics_HeuVar(wf_id=None, plot3D=False)
