...
def runSim_CCPRemappingSingleClusterBasic(run_rm_off=True, run_rm_on=True, seed=1234, multiseeds=False):
    ...
    if(multiseeds==True):
        DATA_FOLDER = "experiment_data/remapping_ccpbased_single_cluster_orig/"+"seed_"+str(seed)+"/"
    else:        
        DATA_FOLDER = "experiment_data/remapping_ccpbased_single_cluster_orig/"
    ...
    if(run_rm_on == True): 
        ...
        #SimParams.CCP_CLUSTER_SIZE = [SimParams.NOC_W, SimParams.NOC_H]
        #SimParams.CCP_SLAVE_MSG_FLOWPAYLOADSIZE = 1
        ...
        SimParams.CCP_LOAN_TASK_AMOUNT = 5
        SimParams.CCP_ENABLE_IMPROVEMENT = False
        SimParams.CCP_REMOTE_SLAVE_HOPCOUNT = SimParams.NOC_W + SimParams.NOC_H 
        # ------------------------------------------------------------
...
if(args.exp_type == "Exp_RemappingBasic"):
    if(args.forced_seed != -1):
        runSim_CCPRemappingSingleClusterBasic(run_rm_off=True, run_rm_on=True, seed=args.forced_seed, multiseeds=True)
    else:
        runSim_CCPRemappingSingleClusterBasic(run_rm_off=True, run_rm_on=True)

else:
    #parser.print_usage()
    #sys.exit("invalid arguments")
    runSim_CCPRemappingSingleClusterBasic(run_rm_off=False, run_rm_on=True)
    #runSim_CCPSimple()
