...
do
	#dirname="experiment_data/remapping_random_initial_mapping/remapping_randombased/seed_${seed}"
	dirname="experiment_data/remapping_randombased/seed_${seed}"
	mkdir -p $dirname
done
...
do
	...
	###########################################
	outname="logs/runexp_remappingbasicrandom_seed_${seed}.out"
	#outname="logs/runexp_remappingbasicrandomNieghbour_seed_${seed}.out"

	nohup python -u RunSim_Exp_RandomRemapping.py -t Exp_RemappingBasic \
