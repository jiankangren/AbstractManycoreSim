...
do
	dirname="experiment_data/remapping_central_dynamic/seed_${seed}"
	mkdir -p $dirname
done
...
do
	...
	###########################################
	outname="logs/runexp_centralfullydynamicmapping_seed_${seed}.out"

	nohup python -u RunSim_Exp_CentralFullyDynamicMapping.py -t Exp_RemappingBasic \
