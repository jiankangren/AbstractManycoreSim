...
do
	dirname="experiment_data/remapping_ccpbased_single_cluster/seed_${seed}"
	mkdir -p $dirname
done
...
do
	...
	###########################################
	outname="logs/runexp_remappingbasicccpsinglecluster_seed_${seed}.out"

	nohup python -u RunSim_Exp_CCPSingleClusterBasedRemapping.py -t Exp_RemappingBasic \
