...
do
	dirname="experiment_data/remapping_ccpbased_orig/seed_${seed}"
	mkdir -p $dirname
done
...
do
	...
	###########################################
	outname="logs/runexp_remappingbasicccp_orig_seed_${seed}.out"

	nohup python -u RunSim_Exp_CCPBasedRemapping_Orig.py -t Exp_RemappingBasic \
