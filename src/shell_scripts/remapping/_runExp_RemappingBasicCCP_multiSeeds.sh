...
do
	dirname="experiment_data/remapping_ccpbased/seed_${seed}"
	mkdir -p $dirname
done
...
do
	...
	###########################################
	outname="logs/runexp_remappingbasicccp_seed_${seed}.out"

	nohup python -u RunSim_Exp_CCPBasedRemapping.py -t Exp_RemappingBasic \
