...
EXP_DATADIR = "experiment_data/hevc_tile_mapping_kaushikTuner/"
...
parser.add_argument("--k_com_sf", help="Kaushiks communication scale factor", type=int, default=-1)
...
if (args.wl_config == None) or (args.forced_seed == -1) or (args.cmbmppri_type == -1) or (args.k_com_sf == -1):    
    sys.exit("Arguments invalid")

# construct filename
runSim_TileMapping_KaushikTuner(
               forced_seed = args.forced_seed,
               cmbmppri_type = args.cmbmppri_type,
               wl_config = args.wl_config,
               memp_select = args.memp_select,
               k_com_sf = args.k_com_sf                           
              )
